...
by silence. Its message set is not implemented.**

DIN SPEC 70121 is a pre-standard whose XSDs are behind a paywall. Hand-transcribing
them would produce a codec with no reference to check against — which is precisely
the failure mode this project exists to avoid.

The generator, the session layer and the timers are all ready for it the day the
schemas are obtainable.
